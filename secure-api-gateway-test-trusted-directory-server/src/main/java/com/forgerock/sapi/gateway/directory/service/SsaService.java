...
public class SsaService {
    ...
    /**
     * ASN.1 OID for the {@code organizationIdentifier} attribute type, defined in
     * <a href="https://www.etsi.org/deliver/etsi_en/319400_319499/31941205/02.02.01_60/en_31941205v020201p.pdf">
     * ETSI EN 319 412-5 §4.2.1</a> (also registered in ITU-T X.520 / RFC 4519 errata).
     * Used when parsing the Subject DN to extract the PSD2 organisation identifier
     * (e.g. {@code PSDGB-FCA-123456}) from issued certificates.
     */
    private static final String OID_ORGANIZATIONAL_IDENTIFIER = "2.5.4.97";

    private static final Logger logger = LoggerFactory.getLogger(SsaService.class);
     ...
     */
    public SsaService(final SsaSigningService ssaSigningService,
                      final SoftwareJwksService softwareJwksService,
     ...
     */
    public String generateSsa(final String certHeader, final SsaRequest request) throws Exception {
    ...
    private X509Certificate parseCertificateFromHeader(final String certHeader) {
        try {
            String pem = URLDecoder.decode(certHeader, StandardCharsets.UTF_8);
            return parsePemCertificate(pem);
        } catch (Exception e) {
            logger.warn("Failed to parse client certificate from ssl-client-cert header", e);
            throw new IllegalArgumentException("Invalid client certificate in ssl-client-cert header", e);
        }
    }

    private Map<String, Object> buildSoftwareFields(final SsaRequest request, final String orgId) {
    ...
    private static X509Certificate parsePemCertificate(final String pem) throws Exception {
    ...
    private Map<String, String> parseDn(final String dn) {
        ...
        try {
            ...
            logger.warn("Failed to fully parse certificate DN '{}': {}", dn, e.getMessage());
        }
        return result;
    }
}
