 ...
 *   <li>{@code GET /health/readiness} — {@code 200} if the JWKS storage file is accessible, {@code 503} otherwise</li>
 * </ul>
 */
public class HealthRouter {
    ...
    public void mount(final Router router) {
        ...
        router.get("/health/readiness").handler(ctx -> {
            boolean storageReady = Files.exists(storageFilePath.getParent() != null
                    ? storageFilePath.getParent() : storageFilePath);
            if (storageReady) {
                ctx.response()
                        .putHeader(HttpHeaders.CONTENT_TYPE, "application/json")
                        .end(new JsonObject().put("status", "UP").encode());
                ...
                ctx.response()
                        ...
                        .end(new JsonObject().put("status", "DOWN")
                                .put("reason", "storage directory not accessible").encode());
            }
        });
    }
}
