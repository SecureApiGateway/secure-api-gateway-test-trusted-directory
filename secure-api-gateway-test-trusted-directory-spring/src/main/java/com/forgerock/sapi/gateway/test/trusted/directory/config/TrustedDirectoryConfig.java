...
public class TrustedDirectoryConfig {
    ...
    public CertificateIssuerService certificateIssuerService(TrustedDirectoryProperties properties) throws Exception {
        TrustedDirectoryProperties.CaProperties ca = properties.getCa();
        KeyStore keyStore = KeyStore.getInstance(ca.getKeystoreType());
        try (InputStream is = new FileInputStream(ca.getKeystorePath())) {
            keyStore.load(is, ca.getKeystorePassword().toCharArray());
        }
        KeyStore.PrivateKeyEntry entry = (KeyStore.PrivateKeyEntry) keyStore.getEntry(
                ca.getKeyAlias(),
                new KeyStore.PasswordProtection(ca.getKeystoreKeyPassword().toCharArray()));
        if (entry == null) {
            throw new IllegalStateException("CA key alias '" + ca.getKeyAlias() + "' not found in keystore");
        }
        PrivateKey caPrivateKey = entry.getPrivateKey();
        X509Certificate caCert = (X509Certificate) entry.getCertificateChain()[0];
        return new CertificateIssuerService(caCert, caPrivateKey, ca.getCertSigningAlg());
    }
}
