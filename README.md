...
|----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|
| TTD_CA_KEYSTORE_PATH             | Path to the keystore containing the CA key                                                                                                            | /secrets/test-trusted-directory/test-trusted-dir.p12 |
| TTD_CA_KEYSTORE_TYPE             | Keystore type                                                                                                                                         | PKCS12                                               |
| TTD_CA_KEYSTORE_ALIAS            | Alias of key in the keystore to use to sign certs issued. Matches the -alias arg supplied to keytool                                                  | ca                                                   |
| TTD_CA_KEYSTORE_STOREPASS        | Keystore store password. Matches the -storepass arg supplied to keytool                                                                               | Passw0rd                                             |
| TTD_CA_KEYSTORE_KEYPASS          | Keystore key password. Matches the -keypass arg supplied to keytool                                                                                   | Passw0rd                                             |
| TTD_ISSUER_NAME                  | The Issuer (iss claim) used in SSAs produced by the Test Trusted Directory.<br/>The AS needs to be configured to trust this issuer for OAuth 2.0 DCR. | Default value: SAPI-G Test Trusted Directory         |
| TTD_SIGNING_KEYSTORE_PATH        | Path to the keystore containing the JWT Signing key                                                                                                   | /secrets/test-trusted-directory/test-trusted-dir.p12 |
| TTD_SIGNING_KEYSTORE_TYPE        | Keystore type                                                                                                                                         | PKCS12                                               |
| TTD_SIGNING_KEYSTORE_ALIAS       | Alias of key in the keystore to use to sign JWTs. Matches the -alias arg issued to keytool                                                            | jwt-signing                                          |
